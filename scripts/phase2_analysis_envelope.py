...
def build(src,out,gdir,gmanifest,forbid_live):
    ...
    m["procom"]=m.procom.map(norm); core,first,second,cu,fu,su=rings(m); cs=candidates(cu,fu,su); selected_name="ADJACENCY_1_PLUS_V1_WALK_GUARD"; selected=cs[selected_name]; acquisition=selected.buffer(EDGE_GUARD_M)
    ...
    if not all([graph_ok,core_ok,first_ok]): raise AssertionError({"graph":graph_ok,"core":core_ok,"first":first_ok})
