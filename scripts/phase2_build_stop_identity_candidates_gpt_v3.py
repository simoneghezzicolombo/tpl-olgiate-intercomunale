...
The script creates review evidence only. It never merges source records, never assigns
final boarding_point_id values and never promotes routing terminals.

Outputs:
1. ASF same-name directional pairs with A/R geometry distances.
2. ASF-to-frozen nearest candidates within the same exact core municipality.
3. Frozen cross-feed numeric-ID alias candidates (Arriva 300xxx vs LineeLecco L00xxx)
   where an ID-suffix crosswalk, close geometry and compatible names provide affirmative
...
def _asf_directional_pairs(asf: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict] = []
    grouped = asf.groupby(
        ["physical_municipality_exact", asf["source_stop_name"].map(_norm_name)],
        dropna=False,
        sort=True,
    )
    pair_id = 0
    for (municipality, norm_name), group in grouped:
...
def _asf_to_frozen_candidates(asf: pd.DataFrame, frozen: pd.DataFrame, radius_m: float, top_k: int) -> pd.DataFrame:
    ...
    for a in asf.to_dict(orient="records"):
        ...
        for f in local.to_dict(orient="records"):
            distance = _haversine_m(
                float(a["lat"]), float(a["lon"]), float(f["lat"]), float(f["lon"])
            )
            if distance <= radius_m:
                similarity = _name_similarity(a["source_stop_name"], f["source_stop_name"])
                candidates.append((distance, similarity, f))
        ...
        for a in numeric.to_dict(orient="records"):
            for b in linee.to_dict(orient="records"):
                ...
                if distance <= 30 and similarity >= 0.85 and same_municipality:
                    status = "STRONG_CROSS_FEED_SOURCE_ALIAS_CANDIDATE"
                else:
                    status = "CROSS_FEED_ID_KEY_REVIEW_REQUIRED"
                rows.append(
                    {
