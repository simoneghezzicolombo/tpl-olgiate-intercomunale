...
def test_no_turn_and_incomplete_nonrejection_is_unknown():
    ...
    assert a.decision(("a",), "c")["allowed"] is True
    assert a.oracle(("a",), "c") is None
    assert a.audit.global_transition_completeness is False


def test_only_turn_complete_domain():
    a = FrozenRT017ViaNodeAdapter(
        [edge("a", "A", "X", "1"), edge("b", "X", "B", "2"), edge("c", "X", "C", "3")],
        [rule("r", "only_right_turn", "X", "1", "2")])
    assert a.audit.global_transition_completeness is True
...
def test_inactive_outside_graph_rule_does_not_govern_complete_graph_transition():
    a = FrozenRT017ViaNodeAdapter(
...
def test_disconnected_is_infeasible_even_when_global_domain_incomplete():
    a = FrozenRT017ViaNodeAdapter(
...
def test_incomplete_adapter_cannot_emit_positive_oracle_even_without_rules():
    a = FrozenRT017ViaNodeAdapter(
        [edge("a", "A", "X", "1"), edge("b", "X", "B", "2")], [],
        unresolved_external_via_way_count=1)
    assert a.decision(("a",), "b")["allowed"] is True
    assert a.oracle(("a",), "b") is None
