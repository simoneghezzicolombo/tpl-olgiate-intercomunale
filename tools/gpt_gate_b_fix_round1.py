...
if bs.count('"population_calibrated_2025": "DERIVED",') != 1:
    raise RuntimeError('population epistemic status matcher failed')
bs = bs.replace('"population_calibrated_2025": "DERIVED",', '"population_calibrated_2025": "ESTIMATE",', 1)
if bs.count('"dem_node_elevation": "DERIVED_FROM_DSM",') != 1:
    raise RuntimeError('DEM epistemic status matcher failed')
...
# trigger retry with exact access block matcher
